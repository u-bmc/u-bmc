...
func (s *SensorMon) handleGetSensor(ctx context.Context, req micro.Request) {
	...
	default:
		...
		req.Error("400", "Invalid sensor identifier", nil)
		return
	}

	if !found {
		s.logger.WarnContext(ctx, "Sensor not found", "request", request.String())
		req.Error("404", "Sensor not found", nil)
		return
	}
...
func (s *SensorMon) readHwmonSensorValue(ctx context.Context, sensorInfo *sensorInfo) error {
	...
	// Convert raw value based on sensor context
	var context v1alpha1.SensorContext
	if sensorInfo.Sensor.Context != nil {
		context = *sensorInfo.Sensor.Context
	}
	value := s.convertHwmonValue(rawValue, context)
...
func (s *SensorMon) convertHwmonValue(rawValue int, context v1alpha1.SensorContext) float64 {
	switch context {
...
func (s *SensorMon) readHwmonThresholds(ctx context.Context, sensorInfo *sensorInfo, analogReading *v1alpha1.AnalogSensorReading) {
	...
	if strings.HasSuffix(baseName, "_input") {
...
func (s *SensorMon) getSensorByContext(context v1alpha1.SensorContext) (*sensorInfo, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	for _, sensor := range s.sensors {
		if sensor.Sensor.Context != nil && *sensor.Sensor.Context == context {
			return sensor, true
		}
	}
	return nil, false

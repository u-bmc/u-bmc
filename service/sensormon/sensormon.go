...
func (s *SensorMon) addHwmonSensor(ctx context.Context, devicePath, deviceName, attribute string, context v1alpha1.SensorContext) error {
	sensorPath := filepath.Join(devicePath, attribute)
	sensorID := fmt.Sprintf("%s_%s", deviceName, attribute)

	// Try to read the label
	labelFile := filepath.Base(attribute)
	...
	if sensorName == "" {
		sensorName = fmt.Sprintf("%s %s", deviceName, attribute)
	}

	sensorContext := context
	sensorStatus := v1alpha1.SensorStatus_SENSOR_STATUS_ENABLED
	sensorUnit := s.getUnitForContext(context)
...
func (s *SensorMon) getUnitForContext(context v1alpha1.SensorContext) v1alpha1.SensorUnit {
	switch context {
...
func (s *SensorMon) registerEndpoints(ctx context.Context) error {
	endpoints := []struct {
		...
		{"sensormon.sensors.list", s.createRequestHandler(s.handleListSensors)},
		{"sensormon.sensor.get", s.createRequestHandler(s.handleGetSensor)},
		{"sensormon.monitoring.start", s.createRequestHandler(s.handleStartMonitoring)},
		{"sensormon.monitoring.stop", s.createRequestHandler(s.handleStopMonitoring)},
		{"sensormon.monitoring.status", s.createRequestHandler(s.handleMonitoringStatus)},
	}
...
func (s *SensorMon) createRequestHandler(handler func(context.Context, micro.Request)) micro.HandlerFunc {

...
func (s *Sensor) LastError() (error, time.Time) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Sensor) readValueWithRetry(ctx context.Context) (Value, error) {
	...
	for attempt := 0; attempt <= s.config.RetryCount; attempt++ {
		if attempt > 0 {
			select {
			case <-ctx.Done():
				return nil, fmt.Errorf("%w: %w", ErrOperationCancelled, ctx.Err())
...
func (s *Sensor) readRawValueWithRetry(ctx context.Context) (string, error) {
	...
	for attempt := 0; attempt <= s.config.RetryCount; attempt++ {
		if attempt > 0 {
			select {
			case <-ctx.Done():
				return "", fmt.Errorf("%w: %w", ErrOperationCancelled, ctx.Err())
			case <-time.After(s.config.RetryDelay):
			}
		}
...
func (s *Sensor) writeRawValue(ctx context.Context, value string) error {
	...
	go func() {
		err := os.WriteFile(s.sensorPath, []byte(value), 0644)
		if err != nil {
			if os.IsNotExist(err) {
...
// HwmonOfNameAndLabel is a convenience function that creates a sensor
// for the specified device name and sensor label (for backward compatibility).
func HwmonOfNameAndLabel(name string, label string) (*Sensor, error) {
	config := NewConfig(
		WithDevice(name),
...
func ReadWithCtx(ctx context.Context, filepath string) (string, error) {
	if ctx == nil {
		return "", ErrNilContext
	}

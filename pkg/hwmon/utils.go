...
func ParseSensorFilename(filename string) (SensorType, int, SensorAttribute, error) {
	...
	for sensorType, pattern := range patterns {
		...
		if len(matches) >= 2 {
			...
			attribute := parseAttributeString(attrStr)
...
func ValidateValueRange(value float64, min, max *float64) error {
	if min != nil && value < *min {
		return fmt.Errorf("%w: value %.3f below minimum %.3f", ErrValueOutOfRange, value, *min)
	}

	if max != nil && value > *max {
		return fmt.Errorf("%w: value %.3f above maximum %.3f", ErrValueOutOfRange, value, *max)
	}

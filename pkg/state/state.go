...
type Machine struct {
	...
	stateEntryCallback  StateEntryCallback
	stateExitCallback   StateExitCallback
...
func (m *Machine) createEntryAction(state string) func(context.Context, ...any) error {
	return func(ctx context.Context, args ...any) error {
		if m.stateEntryCallback != nil {
			if err := m.stateEntryCallback(m.name, state); err != nil {
...
func (m *Machine) createExitAction(state string) func(context.Context, ...any) error {
	return func(ctx context.Context, args ...any) error {
		if m.stateExitCallback != nil {
			if err := m.stateExitCallback(m.name, state); err != nil {
...
func (m *Machine) Start(ctx context.Context) error {
	...
	if m.persistenceCallback != nil {
		...
		if err := m.persistenceCallback(m.name, currentState); err != nil {
...
func (m *Machine) Fire(ctx context.Context, trigger string) error {
	...
	if m.tracer != nil {
		ctx, span = m.tracer.Start(ctx, "state.Fire",
			trace.WithAttributes(
				...
				attribute.String("current_state", m.State()),
			))
		defer span.End()
	}
	...
	if !canFire {
		err := fmt.Errorf("trigger %s cannot be fired from current state %s", trigger, m.State())
		if span != nil {
			span.RecordError(err)
		}
		return err
...
func (m *Machine) handlePostTransition(ctx context.Context, previousState, currentState, trigger string, span trace.Span) {
	// Handle persistence
	if m.persistenceCallback != nil {
		if err := m.persistenceCallback(m.name, currentState); err != nil {
			if span != nil {
	...
	if m.broadcastCallback != nil {
		if err := m.broadcastCallback(m.name, previousState, currentState, trigger); err != nil {
			if span != nil {

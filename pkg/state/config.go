...
type Config struct {
	...
	States []string
	// Transitions defines allowed transitions as simple from->to mappings
	...
	OnStateEntry StateEntryCallback
	// OnStateExit is called when exiting any state
	OnStateExit StateExitCallback
}
...
// StateEntryCallback is called when entering a state.
type StateEntryCallback func(machineName, state string) error

// StateExitCallback is called when exiting a state.
type StateExitCallback func(machineName, state string) error
...
func WithStateEntry(callback StateEntryCallback) Option {
	return &stateEntryOption{callback: callback}
}

type stateExitOption struct {
	callback StateExitCallback
}
...
func WithStateExit(callback StateExitCallback) Option {

...
func (c *Conn) Read(buf []byte) (int, error) {
	...
	return 0, fmt.Errorf("%w: %w", ErrReadFailed, lastErr)
}
...
func (c *Conn) Write(buf []byte) (int, error) {
	...
	return 0, fmt.Errorf("%w: %w", ErrWriteFailed, lastErr)
}
...
func (c *Conn) configure() error {
	...
	if c.config.AddressMode == AddressMode10Bit || c.config.Use10BitAddress {
		if !c.supports10BitAddr() {
			return ErrSMBusNotSupported // 10-bit addressing not supported
		}
...
func (c *Conn) ioctl(cmd, arg uintptr) error {
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, c.file.Fd(), cmd, arg)
	if errno != 0 {
		return errno
	}
	return nil

...
func (c *Conn) ReadVin() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_VIN failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadVout() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_VOUT failed: %w", ErrPMBusDataFormatError, err)
	}

	// Get VOUT_MODE to determine format
	voutMode, err := c.ReadVoutMode()
	if err != nil {
		return 0, fmt.Errorf("%w: failed to read VOUT_MODE: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadIin() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_IIN failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadIout() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_IOUT failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadPin() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_PIN failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadPout() (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_POUT failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) ReadTemperature(sensor uint8) (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: READ_TEMPERATURE_%d failed: %w", ErrPMBusDataFormatError, sensor, err)
	}
...
func (c *Conn) WriteVout(voltage float64) error {
	...
	if err := c.WriteWordData(PMBusVoutCommand, raw); err != nil {
		return fmt.Errorf("%w: VOUT_COMMAND failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) SetVoutOVFaultLimit(voltage float64) error {
	...
	if err := c.WriteWordData(PMBusVoutOVFaultLimit, raw); err != nil {
		return fmt.Errorf("%w: VOUT_OV_FAULT_LIMIT failed: %w", ErrPMBusDataFormatError, err)
	}
...
func (c *Conn) convertDirectToFloat(raw uint16) (float64, error) {
	...
	real := (1.0 / float64(coeff.M)) * (y*math.Pow(10, float64(-coeff.R)) - float64(coeff.B))
	return real, nil
}

// convertFloatToLinear11 converts float64 to PMBus LINEAR11 format.
func (c *Conn) convertFloatToLinear11(value float64) uint16 {
	if value == 0 {
		return 0
	}

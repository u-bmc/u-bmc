...
func (c *Conn) EnterDynamicAddressAssignment() ([]I3CDeviceInfo, error) {
	...
	if err := c.SendByte(I3CCCC_ENTDAA_BC); err != nil {
	...
	for nextAddress <= 0x7F {
		...
		// Assign dynamic address to device
		addressData := []byte{nextAddress << 1} // Address + R/W bit
		if _, err := c.Write(addressData); err != nil {
...
func (c *Conn) SendCommonCommandCode(ccc uint8, data []byte) error {
	...
	cmdBuf := make([]byte, 1+len(data))
	cmdBuf[0] = ccc
	copy(cmdBuf[1:], data)

	if _, err := c.Write(cmdBuf); err != nil {
		return fmt.Errorf("%w: CCC 0x%02x failed: %w", ErrI3CCCCFailed, ccc, err)
	}
...
func (c *Conn) GetDeviceCharacteristics() (bcr, dcr uint8, err error) {
	if c.config.Protocol != ProtocolI3C {
	...
	if err := c.SendCommonCommandCode(I3CCCC_GETBCR, []byte{uint8(c.currentAddr << 1)}); err != nil {
	...
	if err := c.SendCommonCommandCode(I3CCCC_GETDCR, []byte{uint8(c.currentAddr << 1)}); err != nil {
...
func (c *Conn) GetProvisionedID() (uint64, error) {
	...
	if err := c.SendCommonCommandCode(I3CCCC_GETPID, []byte{uint8(c.currentAddr << 1)}); err != nil {
...
func (c *Conn) EnableEvents(events uint8) error {
	...
	data := []byte{uint8(c.currentAddr << 1), events}
	if err := c.SendCommonCommandCode(I3CCCC_ENEC_DC, data); err != nil {
...
func (c *Conn) DisableEvents(events uint8) error {
	...
	data := []byte{uint8(c.currentAddr << 1), events}
	if err := c.SendCommonCommandCode(I3CCCC_DISEC_DC, data); err != nil {
...
func (c *Conn) ResetDynamicAddress() error {
	...
	if err := c.SendCommonCommandCode(I3CCCC_RSTDAA_DC, []byte{uint8(c.currentAddr << 1)}); err != nil {
...
func (c *Conn) ExitHDRMode() error {
	...
	// Send exit pattern (implementation depends on HDR mode)
	// This is a simplified implementation
	exitPattern := []byte{0x00, 0x01} // HDR exit pattern
	if _, err := c.Write(exitPattern); err != nil {
		return fmt.Errorf("%w: failed to exit HDR mode: %w", ErrI3CCCCFailed, err)
	}
...
func (c *Conn) ValidateI3CDevice() error {
	...
	_, _, err := c.GetDeviceCharacteristics()
	if err != nil {

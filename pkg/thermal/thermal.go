...
// ThermalZone represents a thermal management zone with sensors and cooling devices.
...
func InitializeThermalZone(ctx context.Context, zone *ThermalZone) error {
	if zone == nil {
...
func ReadZoneTemperature(ctx context.Context, zone *ThermalZone) (float64, error) {
	if zone == nil {
	...
	if len(temperatures) == 0 {
		return 0, fmt.Errorf("%w: all sensors failed", ErrSensorReadFailure)
	}
...
func UpdatePIDControl(ctx context.Context, zone *ThermalZone, currentTemperature float64) (float64, error) {
	if zone == nil {
		return 0, ErrThermalZoneNotFound
	}
...
func SetCoolingOutput(ctx context.Context, zone *ThermalZone, outputPercent float64) error {
	if zone == nil {
...
func ResetPIDController(ctx context.Context, zone *ThermalZone) error {
	if zone == nil {
		return ErrThermalZoneNotFound
	}
...
func CreateThermalZoneFromProto(proto *v1alpha1.ThermalZone) *ThermalZone {
	if proto == nil {
		return nil
	}

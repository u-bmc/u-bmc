...
func RequestLine(chip, lineName string, opts ...Option) (*gpiocdev.Line, error) {
	...
	gpiocdevOpts = append(gpiocdevOpts, gpiocdev.WithConsumer("u-bmc"))
...
// The lines map should contain line names/numbers as keys and configuration options as values.
...
func ToggleGPIO(chip, lineName string, duration time.Duration, opts ...Option) error {
	...
	if err := line.SetValue(1); err != nil {
		return fmt.Errorf("%w: failed to set GPIO high: %v", ErrOperationFailed, err)
	}

	time.Sleep(duration)

	if err := line.SetValue(0); err != nil {
		return fmt.Errorf("%w: failed to set GPIO low: %v", ErrOperationFailed, err)
	}
...
func GetGPIO(chip, lineName string, opts ...Option) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: failed to read GPIO value: %v", ErrOperationFailed, err)
	}
...
func mapGpiocdevError(err error, context string) error {
	if err == nil {
		return nil
	}
	...
	case err == syscall.ENOENT:
		return fmt.Errorf("%w: %s", ErrChipNotFound, context)
	case err == syscall.EACCES || err == syscall.EPERM:
		return fmt.Errorf("%w: %s", ErrPermissionDenied, context)
	case err == gpiocdev.ErrNotFound:
		return fmt.Errorf("%w: %s", ErrLineNotFound, context)
	case err == gpiocdev.ErrClosed:
		return fmt.Errorf("%w: %s", ErrLineClosed, context)
	default:
		return fmt.Errorf("%w: %s: %v", ErrOperationFailed, context, err)
	}
}
